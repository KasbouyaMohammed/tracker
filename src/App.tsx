@@ .. @@
   return (
-    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 p-4">
+    <div className="min-h-screen bg-gradient-to-br from-white to-amber-50 p-4">
       <div className="max-w-2xl mx-auto">
         {/* Header */}
         <div className="text-center mb-8">
-          <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-green-500 to-blue-500 rounded-full mb-4">
+          <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-amber-600 to-amber-700 rounded-full mb-4">
             <Target className="w-8 h-8 text-white" />
           </div>
-          <h1 className="text-4xl font-bold text-gray-800 mb-2">Daily Habits</h1>
-          <p className="text-gray-600">Build consistency, one day at a time</p>
+          <h1 className="text-4xl font-bold text-amber-900 mb-2">Daily Habits</h1>
+          <p className="text-amber-700">Build consistency, one day at a time</p>
         </div>
 
         {/* Stats Cards */}
         <div className="grid grid-cols-2 gap-4 mb-8">
-          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
+          <div className="bg-white rounded-xl p-6 shadow-sm border border-amber-100">
             <div className="flex items-center justify-between">
               <div>
-                <p className="text-sm text-gray-600 mb-1">Current Streak</p>
-                <p className="text-2xl font-bold text-green-600">{streak} days</p>
+                <p className="text-sm text-amber-600 mb-1">Current Streak</p>
+                <p className="text-2xl font-bold text-amber-700">{streak} days</p>
               </div>
-              <TrendingUp className="w-8 h-8 text-green-500" />
+              <TrendingUp className="w-8 h-8 text-amber-600" />
             </div>
           </div>
-          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
+          <div className="bg-white rounded-xl p-6 shadow-sm border border-amber-100">
             <div className="flex items-center justify-between">
               <div>
-                <p className="text-sm text-gray-600 mb-1">Today's Progress</p>
-                <p className="text-2xl font-bold text-blue-600">{progressPercentage}%</p>
+                <p className="text-sm text-amber-600 mb-1">Today's Progress</p>
+                <p className="text-2xl font-bold text-amber-700">{progressPercentage}%</p>
               </div>
-              <Calendar className="w-8 h-8 text-blue-500" />
+              <Calendar className="w-8 h-8 text-amber-600" />
             </div>
           </div>
         </div>
 
         {/* Progress Bar */}
-        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
+        <div className="bg-white rounded-xl p-6 shadow-sm border border-amber-100 mb-6">
           <div className="flex justify-between items-center mb-3">
-            <h3 className="text-lg font-semibold text-gray-800">Progress</h3>
-            <span className="text-sm text-gray-600">{completedCount}/{habits.length} completed</span>
+            <h3 className="text-lg font-semibold text-amber-900">Progress</h3>
+            <span className="text-sm text-amber-600">{completedCount}/{habits.length} completed</span>
           </div>
-          <div className="w-full bg-gray-200 rounded-full h-3 mb-4">
+          <div className="w-full bg-amber-100 rounded-full h-3 mb-4">
             <div 
-              className="bg-gradient-to-r from-green-500 to-blue-500 h-3 rounded-full transition-all duration-500 ease-out"
+              className="bg-gradient-to-r from-amber-500 to-amber-600 h-3 rounded-full transition-all duration-500 ease-out"
               style={{ width: `${progressPercentage}%` }}
             ></div>
           </div>
-          <p className="text-center text-gray-700 font-medium">{getEncouragementMessage()}</p>
+          <p className="text-center text-amber-800 font-medium">{getEncouragementMessage()}</p>
         </div>
 
         {/* Celebration Message */}
         {showCelebration && (
-          <div className="bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-xl p-6 mb-6 shadow-lg transform animate-pulse">
+          <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-xl p-6 mb-6 shadow-lg transform animate-pulse">
             <div className="text-center">
               <h3 className="text-xl font-bold mb-2">🎉 Congratulations!</h3>
               <p>You've completed all your habits today! Your streak is now {streak} days!</p>
@@ -122,11 +122,11 @@
         }
 
         {/* Habits List */}
-        <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
-          <div className="p-6 border-b border-gray-100">
-            <h3 className="text-lg font-semibold text-gray-800">Today's Habits</h3>
+        <div className="bg-white rounded-xl shadow-sm border border-amber-100 mb-6">
+          <div className="p-6 border-b border-amber-100">
+            <h3 className="text-lg font-semibold text-amber-900">Today's Habits</h3>
           </div>
           <div className="p-6 space-y-4">
             {habits.map((habit) => (
               <div 
                 key={habit.id} 
                 className={`flex items-center p-4 rounded-lg border-2 transition-all duration-200 ${
                   habit.completed 
-                    ? 'bg-green-50 border-green-200' 
-                    : 'bg-gray-50 border-gray-200 hover:border-blue-200 hover:bg-blue-50'
+                    ? 'bg-amber-50 border-amber-200' 
+                    : 'bg-stone-50 border-stone-200 hover:border-amber-200 hover:bg-amber-50'
                 }`}
               >
                 <button
                   onClick={() => toggleHabit(habit.id)}
                   className={`flex-shrink-0 w-6 h-6 rounded-full border-2 mr-4 flex items-center justify-center transition-all duration-200 ${
                     habit.completed
-                      ? 'bg-green-500 border-green-500 text-white'
-                      : 'border-gray-300 hover:border-blue-400'
+                      ? 'bg-amber-600 border-amber-600 text-white'
+                      : 'border-stone-300 hover:border-amber-400'
                   }`}
                 >
                   {habit.completed && <Check className="w-4 h-4" />}
@@ -154,7 +154,7 @@
                 {habit.isEditing ? (
                   <input
                     type="text"
                     defaultValue={habit.name}
-                    className="flex-1 bg-transparent border-none outline-none text-gray-800 font-medium"
+                    className="flex-1 bg-transparent border-none outline-none text-amber-900 font-medium"
                     onBlur={(e) => saveHabitName(habit.id, e.target.value)}
                     onKeyPress={(e) => {
                       if (e.key === 'Enter') {
@@ -167,8 +167,8 @@
                   <span 
                     className={`flex-1 font-medium transition-all duration-200 ${
                       habit.completed 
-                        ? 'text-green-700 line-through' 
-                        : 'text-gray-800'
+                        ? 'text-amber-700 line-through' 
+                        : 'text-amber-900'
                     }`}
                   >
                     {habit.name}
@@ -177,7 +177,7 @@
                 
                 <button
                   onClick={() => startEditing(habit.id)}
-                  className="ml-2 p-2 text-gray-400 hover:text-blue-500 transition-colors duration-200"
+                  className="ml-2 p-2 text-stone-400 hover:text-amber-600 transition-colors duration-200"
                 >
                   <Edit2 className="w-4 h-4" />
                 </button>
@@ -190,7 +190,7 @@
         <div className="text-center">
           <button
             onClick={resetDay}
-            className="inline-flex items-center px-6 py-3 bg-white text-gray-700 font-medium rounded-lg shadow-sm border border-gray-200 hover:bg-gray-50 hover:shadow-md transition-all duration-200"
+            className="inline-flex items-center px-6 py-3 bg-white text-amber-800 font-medium rounded-lg shadow-sm border border-amber-200 hover:bg-amber-50 hover:shadow-md transition-all duration-200"
           >
             <RotateCcw className="w-5 h-5 mr-2" />
             Reset Day